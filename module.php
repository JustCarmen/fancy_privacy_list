	...
	public function modAction($mod_action) {
		switch ($mod_action) {
			case 'admin_config':
				...
				<h2><?php echo $this->getTitle() . ' - ' . $WT_TREE->titleHtml(); ?></h2>
				<table id="privacy_list" class="table table-condensed table-bordered table-striped" style="width:100%">
					<thead>
						<tr>
							<th><span style="float:left"><?php echo WT_I18N::translate('ID'); ?></span></th>
							<th><span style="float:left"><?php echo WT_I18N::translate('Surname'); ?></span></th>
							<th><span style="float:left"><?php echo WT_I18N::translate('Given name'); ?></span></th>
							<th><span style="float:left"><?php echo WT_I18N::translate('Status'); ?></span></th>
							<th><span style="float:left"><?php echo WT_I18N::translate('Privacy settings'); ?></span></th>
							<th><span style="float:left"><?php echo WT_I18N::translate('Explanation'); ?></span></th>
							<th>SURN</th>
							<th>NUMBER</th>
						</tr>
					</thead>
					<tbody>
						...
						<?php foreach ($names as $name): ?>
							...
							if (!$WT_TREE->getPreference('HIDE_LIVE_PEOPLE') && !$settings['RESN']) {
								$auth = $WT_TREE->getPreference('REQUIRE_AUTHENTICATION') ? '(' . WT_I18N::translate('registered users only') . ')' : '';
	...
	private function getPrivacySettings($record) {
		...
		if ($death_dates) {
			...
			if ($keep_alive_death == true) {
				$keep_alive_msg = ' ' . WT_I18N::translate /* I18N: %s is a number */('That is less than %s years ago.', $WT_TREE->getPreference('KEEP_ALIVE_YEARS_DEATH')) . ' ';
			} else {
				if ($keep_alive_birth == true) {
					$keep_alive_msg = ' ' . WT_I18N::translate /* I18N: %s is a number */('This person was born less then %s years ago.', $WT_TREE->getPreference('KEEP_ALIVE_YEARS_BIRTH'));
				}
			}
			$settings = array(
				'RESN'	 => 0,
				'STAT'	 => WT_I18N::translate('Death'),
				'PRIV'	 => $keep_alive ? WT_I18N::translate('Private') : $ACCESS_LEVEL[$WT_TREE->getPreference('SHOW_DEAD_PEOPLE')],
			...
			foreach ($date_matches[1] as $date_match) {
				...
				if ($date->isOK() && $date->MaxJD() <= WT_CLIENT_JD - 365 * $WT_TREE->getPreference('MAX_ALIVE_AGE')) {
					$settings = array(
						...
						'STAT'	 => WT_I18N::translate('Presumed death'),
						'PRIV'	 => $ACCESS_LEVEL[$WT_TREE->getPreference('SHOW_DEAD_PEOPLE')],
						'TEXT'	 => WT_I18N::translate /* %s is a number */('An event occurred in this person\'s life more than %s years ago.', $WT_TREE->getPreference('MAX_ALIVE_AGE')),
					);
					return $settings;
				}
			}
